...
`batched_tiled` is a second Ewald backend designed for heterogeneous batches: atoms are **sum-padded per system** (each system padded to `⌈N_b/BM⌉·BM` atoms, concatenated into one flat array) rather than max-padded to the batch's largest system. The reciprocal sum runs through a pure-JAX `vmap + segment_sum` kernel over fixed-size `(BM × BK)` work tiles. Differentiates cleanly through autograd, including stress, with no `custom_vjp`. Trade-offs vs `batched_mixed`:
