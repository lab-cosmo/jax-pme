...
tiles, so per-system k-vec offset is implicit (`b · K_pad`). Per-system work
still varies — through the atom-tile count, i.e. the number of table rows with
that `b`.
...
makes those segments exactly `n_kvec_tiles`-row contiguous chunks.
"""
...
def phi_recip_xla_vmap(
    ...
    *,
    ...
    def per_triple1(triple):
        ...
        # Invariant (both passes): `b` goes into the slice start, never into an
        # index. Under vmap, `kvec[b]` is a gather that materialises
        # [T, K_pad, 3] -- the whole k-axis per tile, i.e. exactly the matrix
        # the tiling exists to avoid. Pinned by
        # tests/test_batched_tiled_kernel_memory.py.
