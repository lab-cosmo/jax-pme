...
def test_nonorthorhombic_2d_accepted():
    """Non-orthorhombic cells are accepted for 2D PBC."""
    from ase import Atoms

    from jaxpme.batched_mixed.batching import prepare

    cell_sheared = np.array([[10.0, 3.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 20.0]])
    atoms = Atoms(
        "H2",
        positions=[[5.0, 5.0, 5.0], [5.0, 5.0, 6.0]],
        cell=cell_sheared,
    ...
    prepare(atoms, cutoff=3.0)  # should not raise
